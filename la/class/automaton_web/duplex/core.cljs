...
(if csrf-frontend/?csrf-token
  ...
  (core-log/error (ex-info
                   "Fail during realtime component, CSRF token not found"
                   {:cause "?csrf-token does not exist"
                    :data csrf-frontend/?csrf-token})))

(try (let [{:keys [chsk ch-recv send-fn state]}
           (sente/make-channel-socket-client! duplex-route/duplex-uri
                                              csrf-frontend/?csrf-token
                                              {:type :auto ; e/o #{:auto
                                                           ; :ajax :ws}
                                               :packer :edn})]
