...
For an example see https://github.com/ptaoussanis/sente/tree/master/example-project/src/example"
 :start (try (log/info "Starting duplex server component")
             (let [res (start-rt (conf/read-param [:env]))]
               (log/trace "Duplex server component is started")
               res)
             (catch Exception e (log/error "Unexpected error during duplex start" e)))
