(ns automaton-web.middleware
  ...
  (:require [automaton-core.log :as log]
            ...
            [automaton-web.pages.errors :as errors]
...
(defn wrap-transit
  ...
  [handler]
  (fn [request] (let [response (handler request)] (ring-response/header response "Accept" "application/transit+json"))))

(defn wrap-deny-frame
  "Disallow iFrame"
  [handler]
  (fn [request] (let [resp (handler request)] (ring-response/header resp "X-Frame-Options" "DENY"))))
...
(defn wrap-gzip "Compress response before sending to the client. Needs to be called last" [handler] (ring-gzip/wrap-gzip handler))
...
(defn wrap-keyword-params "Translate string keys in the :params to keywords" [handler] (ring-keyword-params/wrap-keyword-params handler))

(defn wrap-cookies ([handler] (ring-cookies/wrap-cookies handler)) ([handler options] (ring-cookies/wrap-cookies handler options)))

(def parameters-middleware "map :query-params, :form-params, :params -> reitit wrapper for ring wrap-params" rrmp/parameters-middleware)
...
(defn wrap-translator "Insert the :tr key for translation" [web-translate] (be-translator/wrap-translator web-translate))
...
(defn- wrap-exception-handling*
  ...
  ([handler request body-fn]
   (try (handler request)
        (catch Exception e (log/error (ex-info "Fail in sync middleware." {:error e})) (body-fn))
        (catch Error e (log/fatal (ex-info "Fail in sync middleware." {:error e})) (body-fn))))
  ([handler request respond raise body-fn]
   (try (handler request respond raise)
        (catch Exception e (log/error (ex-info "Fail in async middleware." {:error e})) (respond (body-fn)))
        (catch Error e (log/fatal (ex-info "Fail in async middleware." {:error e})) (respond (body-fn))))))
...
(defn wrap-exception-handling
  ...
  * (optional) `body` - defaults to building internal error page, but could be supplied with anything suitable."
  ([handler]
   (fn
     ([request] (wrap-exception-handling* handler request #(http-response/internal-server-error (errors/internal-error-page request))))
     ([request respond raise]
      (wrap-exception-handling* handler
                                request
                                respond
                                raise
                                #(http-response/internal-server-error (errors/internal-error-page request))))))
  ([handler body]
   (fn
     ([request] (wrap-exception-handling* handler request #(http-response/internal-server-error body)))
     ([request respond raise] (wrap-exception-handling* handler request respond raise #(http-response/internal-server-error body))))))
