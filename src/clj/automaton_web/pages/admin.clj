  ...
  (let [gh-token (env-vars/get-env "GH_TOKEN")
        rows (reduce (fn [acc app]
                       (let [app-name (get-in app [:app-name])
                             ...
                             prod-env (ignore-error #(request/http-get (str (get-in app [:build-config :run-env :prod-env :web-link])
                                                                            "/api/version"))
                                                    (atom "Error"))
                             test-env (ignore-error #(request/http-get (str (get-in app [:build-config :run-env :test-env :web-link])
                                                                            "/api/version"))
                                                    (atom "Error"))]
                         (conj acc
