(ns automaton-web.pages.admin
  "Admin page"
  (:require [automaton-core.adapters.build-config :as build-config]
...
(defn menu-items
  ...
  [{:message "Docker hub"
   ...
   {:message "Project tasks"
    :category "Organization"
    :uri "https://dev.azure.com/mateusz0216/Hephaistox%20Agile/_boards/board/t/Hephaistox%20Agile%20Team/Stories"}
   {:message "Shared files with team"
    :category "Organization"
    :uri "https://hephaistox.sharepoint.com/sites/hephaistox/Shared%20Documents/Forms/AllItems.aspx"}
...
(defn sorted-items "Create categories of menu item" [m] (sort (group-by :category m)))
...
(defn component-graph-deps
  ...
  (map (fn [m]
         (-> m
             (dissoc :order)
             (assoc :status-color (if (contains? (:status m) :started) :green :red))))
       (sort-by :order graph)))

(defn main-app-build-config-view
  [build-config-edn]
  (let [app-name (:app-name build-config-edn)] [:div [:div (str app-name " full build config:")] (table/map->table build-config-edn)]))

(defn display-links
  [items-by-category]
  [:div {:class ["flex flex-col"]} [:h2 "Links"]
   (for [[category items] items-by-category]
     (when category
       [:div [:h3 {:class ["text-left"]} category]
         ...
         (for [{:keys [uri message]} items]
           [:a
            {:class ["cursor-pointer select-none"]
             :href uri
             :target "blank"}
            [:div {:class ["flex flex-row border-black border-solid border-2 m-2"]} [:div {:class ["m-2 p-2"]} message]]])]]))])

(defn all-apps-links
  [cust-apps]
  (let [admin-maps (reduce (fn [acc app]
                             (let [app-name (:app-name app)
                                   app-repo-link (get-in app build-config/build-config-repo-link)
...
(defn- ignore-error [fn-try ret-val] (try (or (fn-try) ret-val) (catch Exception _ ret-val)))

(defn envs-indication
  [cust-apps]
  (let [gh-token (env-vars/get-env "GH_TOKEN")
        rows (reduce (fn [acc app]
                       (let [app-name (get-in app build-config/repo-name)
                             local-env (get (version/slurp-version (str app-name "/")) version/release)
                             gh-version (ignore-error #(request/http-get
                                                        (str "https://api.github.com/repos/hephaistox/" app-name "/contents/version.edn")
                                                        {:headers {"Authorization" (str "Bearer " gh-token)
                                                                   "Accept" "application/vnd.github.raw+json"}})
                                                      (atom "Error"))
                             gh-version-waited (ignore-error #(:release (read-string (:body @gh-version))) "Error")
                             prod-env (ignore-error #(request/http-get (str (get-in app build-config/prod-web-link) "/api/version"))
                                                    (atom "Error"))
                             test-env (ignore-error #(request/http-get (str (get-in app build-config/test-web-link) "/api/version"))
                                                    (atom "Error"))]
                         (conj acc
                               [app-name local-env gh-version-waited (:body @prod-env) (:body @test-env)
                                (if (= local-env gh-version prod-env test-env)
                                  (web-icons/icon {:path-kw :svg/check-circle
                                                   ...
                                                   :class ["w-[20px]
                                                    h-[20px]"]
                                                   :hover? false})
                                  (web-icons/icon {:path-kw :svg/x-circle
                                                   :scale 0.03
                                                   :color "red"
                                                   :class ["w-[20px]
                                                    h-[20px]"]
                     ...
                     []
                     cust-apps)]
    (table/table {:headers ["App name" "Local env" "Github" "Prod env" "Test env" "Status"]
                  :rows rows})))

(defn current-app-stats
  [graph]
  [:div {:class ["flex flex-wrap"]}
   [:div [:h2 "Component graph deps"]
...
(defn index
  ...
  (let [items-by-category (sorted-items (menu-items))
        ...
        app-name (conf/read-param [:app-name])
        build-configs-paths (build-config/search-for-build-config)
        build-configs (map edn-utils/read-edn build-configs-paths)
        current-app-config (first (filter (fn [app] (= (:app-name app) app-name)) build-configs))
        cust-apps (filter (fn [app] (:cust-app? app)) build-configs)]
    [:div
     {:id "admin-index"
      :class ["text-center w-full"]} (envs-indication cust-apps) (display-links items-by-category) (all-apps-links cust-apps)
     (current-app-stats graph) (main-app-build-config-view current-app-config)]))
