...
(defn main-app-build-config-view
  ...
  (let [app-name (:app-name build-config-edn)]
    [:div [:div (str app-name " full build config:")] (web-table/map->table build-config-edn)]))
        ...
        [:div {:class ["flex flex-wrap"]}
         (for [{:keys [uri message]} items]
           [:a {:class ["cursor-pointer select-none"]
                :href uri
                :target "blank"}
            [:div {:class ["flex flex-row border-black border-solid border-2 m-2"]}
             [:div {:class ["m-2 p-2"]}
              message]]])]]))])
...
(defn envs-indication
  ...
  (let [gh-token (env-vars/get-env "GH_TOKEN")
        ...
        (reduce
         (fn [acc app]
           (let [app-name (get-in app [:app-name])
                 ...
                 gh-version (ignore-error #(request/http-get
                                          ...
                                          (atom "Error"))
                 gh-version-waited (ignore-error #(:release (read-string (:body @gh-version)))
                                                 "Error")
                 prod-env (ignore-error #(request/http-get
                                          (str (get-in app
                                                       [:build-config :run-env :prod-env :web-link])
                                               "/api/version"))
                                        (atom "Error"))
                 test-env (ignore-error #(request/http-get
                                          (str (get-in app
                                                       [:build-config :run-env :test-env :web-link])
                                               "/api/version"))
                                        (atom "Error"))]
             (conj
              acc
         ...
         []
         cust-apps)]
    (web-table/table {:headers ["App name" "Local env" "Github" "Prod env" "Test env" "Status"]
                      :rows rows})))
