(ns automaton-web.pages.index
  (:require [hiccup2.core :as h.page]
    ...
    [:html
     [:head [:meta {:charset "utf-8"}]
      ...
      [:link
       {:type "text/css"
        :rel "stylesheet"
        :href "/css/compiled/styles.css"}] (when header-elements (for [header-el header-elements] header-el))
      [:script (h.page/raw (format "var _envVars =%s" (ws-util/mapToJSmap envVars)))] [:title app-name]]
     (vec (concat [:body (anti-forgery/anti-forgery-html-token) (for [el body] el)]))])))
