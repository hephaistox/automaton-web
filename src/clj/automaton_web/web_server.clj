...
(Thread/setDefaultUncaughtExceptionHandler (reify
                                            Thread$UncaughtExceptionHandler
                                              (uncaughtException [_ thread ex]
                                                (log/error (ex-info (str "Uncaught exception on" (.getName thread)) {:error ex})))))
...
(defn start-server
  ...
  (let [http-port (conf/read-param [:http-server :port])
    ...
    (try (http-kit/run-server handler webserver-opts) (catch Exception e (log/error (ex-info "Unable to start server" {:error e}))))))

(defn stop-server [server] (server) (log/info "Server stopped") server)
