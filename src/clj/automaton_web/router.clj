(ns automaton-web.router
  ...
  (:require [automaton-web.duplex.routes :as duplex-routes]
...
(defn router
  ...
  (reitit-ring/router (vector web-routes [@duplex-routes/routes {:compile coercion/compile-request-coercers}])
...
(defn ring-handler
  ...
  * `translator-middlewares` middlewares needed for translator"
  ...
  (reitit-ring/ring-handler (router web-routes web-middleware)
                            (reitit-ring/routes (bewh/resource-handler {}) (bewh/default-handlers default-handlers translator-middlewares))
