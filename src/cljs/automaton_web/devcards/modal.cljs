...
(defcard modal
         (bdu/wrap-component [:div {:class ["flex flex-col gap-4"]} [:div "big modal"]
                              (sut/wrap-modal-call {:modal-id "bigModal"} [bcb/button {:text "Call modal"}])
                              [sut/modal-big
                               {:title [:div "hello"]
                                :body [:div "Body"]
                                :id "bigModal"}]]))
...
(defcard
 details-modal
 (bdu/wrap-component
  [:div [:h3 "basic-version"] (sut/wrap-modal-call {:modal-id "basic-modal"} [bcb/button {:text "Show modal"}])
   [sut/details-modal
    {:img
     ...
     :modal-id "basic-modal"}] [:h3 "With sections"] (sut/wrap-modal-call {:modal-id "sections-modal"} [bcb/button {:text "Show modal"}])
   [sut/details-modal
    {:img
     "https://images.unsplash.com/photo-1519345182560-3f2917c472ef?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=facearea&facepad=8&w=1024&h=1024&q=80"
     :name "Jar Jar Binks"
