...
(defn -init-lang
  ...
  * `main-lang`"
  ...
  (if-let [language (or par-lang @cookies-lang_ main-lang)]
    (do (log/debug "Init language " language) language)
    (let [default-language (-> (default-languages this)
                               first)]
      (log/warn "No language found in the url, default to " default-language)
      default-language)))

(defn -lang
  "By default, the frontend get its language value from `::web-subs/lang`"
  [this]
  (try (some-> (bwr/subscribe [::web-subs/lang])
               deref)
       (catch :default _ (log/warn "The language event has not been found, default language is used") (default-languages this))))
