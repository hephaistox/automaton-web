  ...
  (if (vector? ?data)
    (let [[old-state-map new-state-map] ?data]
      (if (:first-open? new-state-map)
        ...
        (do (log/trace "Channel socket state change: " old-state-map) (log/trace "Channel socket state change: " new-state-map))))
    (log/error (ex-info "Data should be of vector type"
...
(defmethod -event-msg-handler [:chsk/handshake nil]
  [{:as _ev-msg
    :keys [?data]}]
  (let [[_?uid _?csrf-token _?handshake-data] ?data] (log/trace "Handshake: " ?data)))
