...
(defn component
  ...
  * field-name is the name of the field where to post the file in the post http message"
  ...
  (if (and tag uri urifield)
    (let [files (bwr/subscribe [::files tag])
          upload-message (bwr/subscribe [::upload-message tag])
          network-error (bwr/subscribe [::network-error tag])
          loading? (bwr/subscribe [::loading? tag])]
        ...
        [:div {:class ["flex flex-row"]}
         (let [nb-files (if @files (.-count @files) 0)]
           [:label {:for "image-upload"}
            (if (= 0 nb-files) message (str "File" (when (> 1 nb-files) "s") ": (" (str/join ", " (for [file @files] (.-name file))) ")"))])
         (when @loading?
           [:div {:class ["flex justify-center items-center inline-block ml-2 float-right"]}
            [:div
             {:class ["spinner-border animate-spin inline w-8 h-8 border-4 rounded-full"]
...
file:py-3 file:px-5 file:mr-5 file:text-body-color file:cursor-pointer file:hover:bg-primary file:hover:bg-opacity-10"]
           :on-change #(bwr/dispatch [::files-selected tag
                                      (-> %
                                          .-target
                                          .-files) uri urifield])})]
        (when @network-error
...
(bwr/reg-event-fx ::files-selected
...
(bwr/reg-event-db ::file-upload-success (fn-traced [db [_ tag]] (assoc-in db [:file-upload tag :success] true)))

(bwr/reg-event-fx ::trigger-upload
 ...
 (fn-traced [{:keys [db]}
            ...
            {:db (assoc-in db [:file-upload tag :loading?] false)
             :fx [[:dispatch [::reset-selector tag]]
                  (if (= "done" status) [:dispatch [::new-file-uploaded tag upload-response]] [:dispatch [::file-upload-error tag]])]}))

(bwr/reg-event-fx ::error-message
...
(bwr/reg-event-db ::reset-selector
                  (fn-traced [db [_ tag]]
                             (let [image-upload (. js/document (getElementById "image-upload"))]
                               (set! (.-value image-upload) "")
                               (assoc-in db [:file-upload tag :files] nil))))

(bwr/reg-event-db ::file-upload-error (fn-traced [db [_ tag]] (assoc-in db [:file-upload tag :error?] true)))

(bwr/reg-sub ::files (fn [db [_ tag]] (get-in db [:file-upload tag :files])))

(bwr/reg-sub ::upload-message (fn [db [_ tag]] (get-in db [:file-upload tag :upload-error])))

(bwr/reg-sub ::network-error (fn [db [_ tag]] (get-in db [:file-upload tag :network-error])))

(bwr/reg-sub ::loading? (fn [db [_ tag]] (get-in db [:file-upload tag :loading?])))
