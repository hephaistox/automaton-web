...
file:py-3 file:px-5 file:mr-5 file:text-body-color file:cursor-pointer file:hover:bg-primary file:hover:bg-opacity-10"]
           :on-change #(web-events-proxy/dispatch [::files-selected tag
                                                   (-> %
                                                       .-target
                                                       .-files) uri urifield])})]
        (when @network-error
...
(web-events-proxy/reg-event-db ::file-upload-success (fn-traced [db [_ tag]] (assoc-in db [:file-upload tag :success] true)))
...
(web-events-proxy/reg-event-fx ::error-message
...
(web-events-proxy/reg-event-db ::file-upload-error (fn-traced [db [_ tag]] (assoc-in db [:file-upload tag :error?] true)))

(web-events-proxy/reg-sub ::files (fn [db [_ tag]] (get-in db [:file-upload tag :files])))

(web-events-proxy/reg-sub ::upload-message (fn [db [_ tag]] (get-in db [:file-upload tag :upload-error])))

(web-events-proxy/reg-sub ::network-error (fn [db [_ tag]] (get-in db [:file-upload tag :network-error])))
