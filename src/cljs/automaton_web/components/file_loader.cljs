(ns automaton-web.components.file-loader
  ...
  (:require
   ...
   [automaton-web.security.csrf-frontend :as session]))
  ...
  * field-name is the name of the field where to post the file in the post http message"
  ...
  (if (and tag uri urifield)
    (let [files (bwr/subscribe [::files tag])
      ...
      [:div
       {:class ["flex flex-wrap -mx-4 file-loader"]}
       [:div {:class ["lg:w-5/12"
                      "w-full px-4 mb-12"]}
        [:div
         {:class ["flex flex-row"]}
         ...
         (when @loading?
           [:div
            {:class ["flex justify-center items-center inline-block ml-2 float-right"]}
            [:div
...
file:py-3 file:px-5 file:mr-5 file:text-body-color file:cursor-pointer file:hover:bg-primary file:hover:bg-opacity-10"]
                 :on-change #(bwr/dispatch [::files-selected tag (->  % .-target .-files) uri urifield])})]
        (when @network-error
