(ns automaton-web.components.form
  (:require
   ...
   [automaton-core.utils.uuid-gen :as buug]
...
(defn initialize-state
  ...
  (let [values (or (merge initial-values
                          initial-touched)
                   {})
    ...
    (if-let [user-provided-state state]
      (do (swap! user-provided-state
                 (fn [db]
                   (merge initialized-state db)))
          user-provided-state)
      (bwc/ratom initialized-state))))
...
(defn disable
  [state & [ks]]
  (swap! state update :disabled?
         #(apply conj ((fnil into #{}) %) ks)))
...
(defn dirty
  ...
  (first (data/diff values
                    (or initial-values
                        {}))))

(defn on-submit
  [evt {:keys [state on-submit
               validation reset]}]
  (.preventDefault evt)
...
(defn form
  ...
  (let [state (initialize-state props)
        form-id (or (:form-id props) (str (buug/unguessable)))
        ...
        (let [validation (when-let [val-fn (:validation props)]
          ...
          [component
           {:props (:props props)
            ...
            :attempted-submissions (or (:attempted-submissions @state)
                                       0)
            :successful-submissions (or (:successful-submissions @state)
                                        0)
...
(defn append-form [form-id el]
  (.appendChild (.getElementById js/document form-id) el))

(defn submit-form [form-id]
  (-> js/document
...
(defn form-basic
  ...
  [{:keys [form-id on-submit validation action method component-did-mount text]
    :or {form-id (str (buug/unguessable))
         text (bilf/btr :submit)}}
   & elements]
  (let [submit-fn (or on-submit
                      #(submit-form form-id))]
