                   ...
                   (or invalid?
                       (when required?
                         (and touched
                              (touched name)
                              (get errors name)))))
        e-msg (or error-message
                  (get errors name))]

    [:div
     {:class [(when (= size :full)
                "sm:col-span-2")]}
     [:div
      ...
      (when invalid?
        [:div
         ...
         [bci/icon {:class ["icon-red"]}]])]
     (when invalid?
       [:p
        {:class ["ml-2 mt-2 text-sm text-red-600"]}
        e-msg])]))
...
(defn checkbox
  ...
  [{:keys [id title description name size touched errors values error-message required? invalid?] :as params}]
  (let [invalid? (or invalid?
                     (when required? (and touched (touched name) (get errors name))))
  ...
  [text-field
   (merge params
          {:id id
           ...
           :text (bilf/btr :email)
           :type "email"
           :placeholder (bilf/btr :example "mati@hephaistox.com")
           :autocomplete "email"})])
...
(defn first-name-field
  ...
  [text-field
   (merge params
          {:id id
           :name name
           :text (bilf/btr :first-name)
           :placeholder (bilf/btr :example "Pierre")
...
(defn last-name-field
  ...
  [text-field
   (merge params
          {:id id
           :name name
           :placeholder (bilf/btr :example "Caumond")
           :text (bilf/btr :last-name)
...
(defn company-field
  ...
  [text-field
   (merge params
          {:id id
           :name name
           :text (bilf/btr :company)
           :placeholder (bilf/btr :example "Michelin")
...
(defn checkboxes
  ...
  [:fieldset
   ...
   [:div
    ...
    (for [checkbox-params checkboxes-params]
