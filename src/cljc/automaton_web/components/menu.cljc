...
(defn- horizontal-version
  ...
  (vec (concat [:div {:class [(when-not force-burger? "xl:block") "flex flex-col xl:flex-row gap-1 hidden"]}] items)))
   ...
   [:div {:class ["absolute" (if (= :left burger-position) "left-10" "right-10")]}
    [:div {:class ["overflow-hidden hover:h-fit hover:w-fit w-5 relative h-8"]}
     [:div {:class ["flex flex-col absolute" (if (= :left burger-position) "left-0" "right-0")]}
      [:div {:class ["cursor-pointer"]} [web-icons/icon {:path-kw :svg/burger}]]]
     (vec (concat [:div {:class ["h-fit flex flex-col mt-10 max-w-[250] border shadow shadow-xl bg-white"]}] items))]]])
...
(defn component
  ...
  * `burger-position` where the minified version (burger) is shown on the remaining space, `:left` or `:right`
  ...
  (let [auto-select? (not (nil? path))
        items (vec (for [{:keys [href]
                          :as item}
                         items]
                     (let [menu-match? (when (and (string? path) (string? href)) (let [uri path] (= uri href)))]
                       [menu-item/component
                        (merge (assoc item :path path :auto-select? auto-select?) (when auto-select? {:selected menu-match?}))])))
        params (assoc params :items items)]
    [:div {:class ["w-full z-10"]} [horizontal-version params] [burger-version params]]))
