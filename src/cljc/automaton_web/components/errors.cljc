(ns automaton-web.components.errors
  ...
  (:require [automaton-web.components.navigation :as bcn]))

(defn not-found [{:keys [not-found-page-title
                         not-found-page-description
                         back-home-text
                         back-link]
                  :or {back-link "/"}}]
  [:main
   {:class ["relative isolate min-h-full h-full"]}
   [:div {:class ["before:content-[''] before:absolute before:h-full before:w-full before:bg-black/[.5]"]}
    [:img
   ...
   [:div
    ...
    [:p
     {:class ["text-base font-semibold leading-8 text-white text-border-red"]}
     "404"]

    [:h1
     {:class ["mt-4 text-3xl font-bold tracking-tight text-white sm:text-5xl text-border-red-thin"]}
     not-found-page-title]
    [:p {:class ["mt-4 text-base text-white/70 sm:mt-6 text-border-black"]}
     not-found-page-description]
    [:div
     {:class ["mt-10 flex justify-center text-border-black"]}
     (bcn/back-navigation {:href back-link
                           :text back-home-text
                           :dark? true})]]])
