...
(defn card
  ...
  [:div
   (merge {:class ["rounded-2xl px-8 py-10 cursor-pointer flex flex-col text-center"
                   (if dark? "bg-theme-light hover:bg-orange-100 drop-shadow-md" "bg-theme-dark hover:bg-orange-300 drop-shadow-md")]
   ...
   [:img
    {:class ["mx-auto h-24 w-24 rounded-full"]
     ...
     :alt ""}]
   [:h3
    {:class ["mt-6 font-bold" (if dark? "text-theme-dark" "text-theme-light")]
     :style {:fontFamily "'Plus Jakarta Sans'"}} name]
   [:p
    {:class ["text-sm leading-6" (if dark? "text-theme-dark-secondary" "text-theme-light-secondary")]
     :style {:fontFamily "'Plus Jakarta Sans'"}} title]
   [:ul
