...
(defn- footer-link
  [{:keys [title items dark? disabled?] :as _footer-category}
   class]
  [:div {:class class}
   [:h3
              ...
              [:li
               ...
               [:a {:href href
                    ...
                    (if dark?
                      ...
                      (if disabled?
                        "text-gray-900"
                       "text-gray-600 hover:text-gray-900"))]} text]])))])
...
(defn- social-networks-comp [social-networks dark?]
  (for [[icon-key icon-comp] social-networks]
    ^{:key (str "icon-" icon-key)}
    (assoc-in icon-comp [1 :dark?] dark?)))
...
(defn footer
  ...
  * `version` is the string to show the version
  ...
  [:footer
   {:class [(if dark? "bg-gray-900" "bg-white")]
    :aria-labelledby "footer-heading"}
   [:div
    {:class ["mx-auto max-w-7xl py-12 px-4 sm:px-6 lg:py-16 lg:px-8"]}
    [:div
     {:class ["xl:gap-8 xl:grid xl:grid-cols-3"]}
     [:div
      {:class ["xl:col-span-1 space-y-8"]}
      [:img
       {:src badge
        :alt company-name
        :class ["h-10"]}]
     ...
     (let [[footer1 footer2 footer3 footer4] footer-lists]
       [:div
        {:class ["grid grid-cols-2 gap-8 xl:col-span-2 mt-12 xl:mt-0"]}
        [:div
         {:class ["md:grid md:grid-cols-2 md:gap-8"]}
         [footer-link (merge footer1
                             {:dark? dark?})]
         [footer-link
          (merge footer2
                 {:dark? dark?})
        ...
        [:div
         ...
         [footer-link
          ...
          ["mt-12 md:mt-0"]]]])]
    [:div
     {:class ["mt-12 border-t border-gray-200 pt-8 relative flex items-center"]}
     (when release
       [bcv/component {:version release :dark? dark?}])
     [:p
      {:class ["text-base xl:text-center w-full"
               (if dark?
                 "text-gray-400"
                 "text-gray-500")]}
      title]]]])
   ...
   [:div
    ...
    [:nav
     ...
     (for [footer footer-lists]
       ^{:key (str "simple-footer-link-" (:id footer))}
       [simple-footer-link
        (merge
         {:dark? dark?}
         footer)])]
    ...
    (into [:div
           {:class ["mt-10 flex justify-center space-x-10 relative"]}
           (when release
             [bcv/component {:version release :dark? dark?}])]
          (social-networks-comp social-networks dark?))

    [:p
     {:class ["mt-10 text-center text-xs leading-5"
              (if dark?
                "text-gray-400"
                "text-gray-500")]}
     title]]])
