...
(def icons-path
  ...
  {:svg/exclamation {:original-width 512
   ...
   :svg/check-circle {:original-width 512
                      :original-height 512
                      :fill "orange"
   ...
   :svg/x-circle {:original-width 512
                  :original-height 512
   ...
   :svg/checked {:original-width 512
                 :original-height 512
   ...
   :svg/twitter {:original-width 512
                 :original-height 512
...
(defn icon
  ...
  * width is the width of the icon, 13px by default
  * height is the height of the icon, equal to the width by default
  * path-kw is one of the keyword defined in icons-path
  * color-fill is the content of the icon
  * color-stroke is the boundary of the icon"
  [{:keys [width height path-kw class href dark?]
    :or {width 20
         height width
         path-kw :svg/exclamation}}]
  (let [{:keys [original-width
                original-height
                fill
                scale
                path]
         :as _icons} (get icons-path path-kw)
        svg [:svg {
                   :width width
                   :stroke "currentColor"
                   :href href
                   :fill (if fill fill (if dark? "white" "black"))
                   :class (vec
                           (concat ["relative"
                                    "svg-hover-orange"
                                    "text-black"]
                   ...
                   :aria-hidden "true"
                   :view-box (str "0 0 "
                                  (or original-width width)
                                  " "
                                  (or original-height height))
                   :xmlns "http://www.w3.org/2000/svg"}
             [:g {:transform (str "scale(" (or scale 1) ")")}
              [:path {:d path}]]]]
