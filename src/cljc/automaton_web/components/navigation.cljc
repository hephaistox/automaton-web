(ns automaton-web.components.navigation "Navigation component,
...
(defn back-navigation
  ...
  * The linked could be set with `href` or `on-click` and are higher priority"
  ...
  [:a
   (merge (cond href {:href href}
                on-click {:on-click on-click})
          {:class ["font-semibold leading-7" (when (some nil? [href on-click]) "cursor-pointer")
                   (if dark? "text-additional" "text-primary")]}) [:span {:aria-hidden "true"} "← "] text])
