...
(defn section-text-button
  ...
  [:div {:class ["mx-auto grid max-w-7xl grid-cols-1 gap-10 px-6 lg:grid-cols-12 lg:gap-8 lg:px-8"]}
   ...
   [:div {:class ["m-auto lg:col-span-3 flex justify-center items-center"]} (if button button [web-button/button {:text btn-text}])]])
...
(defn section-text-video
  ...
  [:div
   ...
   [:div {:class ["flex flex-col gap-8"]}
    ...
    [web-button/link-button {:link btn-link} (merge btn-props {:class ["hidden lg:block"]})]]
   [:iframe
    {:frameBorder "0"
     :scrolling "no"
     :class ["w-full aspect-video pt-8 lg:pl-8"]
     :allowFullScreen true
     :src video-src}] [web-button/link-button {:link btn-link} (merge btn-props {:class ["lg:hidden block w-full mt-8"]})]])

(defn- card-details-modal
  [{:keys [img name description title modal-id sections]}]
  [apply web-modal/details-modal
   {:img img
    :name name
    :description description
    :title title
    :modal-id modal-id} sections])
...
(defn section-clickable-cards-modal
  ...
  * section - [optional] section description map with :title :description"
  ...
  [section-full-container {:dark? dark?} [section-description (merge section {:dark? dark?})]
   [web-grid-list/grid-box
    {:size size
     :class ["mt-8"]}
