...
(defn section-full-container
  ...
  [:div {:class (vec (concat ["h-fit block p-8" (if dark? "bg-theme-dark" "bg-theme-light")]
                             class))}
...
(defn section-text-container
  ...
  [:div {:class (vec (concat ["mx-auto max-w-3xl text-base leading-7"] class))}
   (into [:div] (for [component components] ^{:key (uuid-gen/unguessable)} component))])
...
(defn section-description
  ...
  [:div {:class ["mx-auto sm:text-center pt-4"]}
   [:h2 {:class ["text-3xl font-bold tracking-tight sm:text-4xl"
    ...
    title]
   [:p {:class ["mt-6 max-w-2xl text-lg leading-8 m-auto mt-5"
...
(defn section-text-button
  ...
  [:div {:class ["mx-auto grid max-w-7xl grid-cols-1 gap-10 px-6 lg:grid-cols-12 lg:gap-8 lg:px-8"]}
   [:div {:class ["max-w-xl text-3xl font-bold tracking-tight sm:text-4xl lg:col-span-7 flex"
                  (if dark? "text-theme-light" "text-theme-dark")]}
   ...
   [:div {:class ["flex flex-col gap-8"]}
    [:div {:class ["justify-self-center pr-4"]}
     ...
     [:figcaption {:class ["mt-8 text-base text-end"]}
      [:div {:class ["font-semibold" (if dark? "text-theme-light" "text-theme-dark")]}
       subtitle]
      [:div {:class ["mt-1" (if dark? "text-theme-light-secondary" "text-theme-dark-secondary")]}
       text]]]
