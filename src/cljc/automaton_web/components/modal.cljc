   ...
   [:div
    ...
    [:div
     ...
     [:div
      ...
      title
      [web-button/x-button {:btn-props close-modal-prop}]]
...
(defn details-modal
  ...
  :header being composite of name and title.
  ...
  [modal-big
   {:title [:div {:class ["flex gap-4 items-center grid"]}
            [:h3 {:class
                  ["text-2xl font-bold leading-8 tracking-tight text-gray-900"]}
             name]
    ...
    [:div
     {:class
      ["relative overflow-y-auto p-4 text-base leading-7 text-gray-700 mb-8"]}
     [:img
      {:class
       ["float-left aspect-[4/5] w-36 flex-none rounded-2xl object-cover mb-1 mr-4"]
       :src img
